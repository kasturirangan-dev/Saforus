import { PiracyDetectionRequest } from './interface';

const mockoOrders: Partial<PiracyDetectionRequest>[] = [
  {
    id: 1,
...
export function mockFetchOrderList(
  query?: Partial<PiracyDetectionRequest>
): Promise<Partial<PiracyDetectionRequest>[]> {
        ...
        return resolve(
          mockoOrders.filter((order) => {
            return Object.keys(query).every((key) => {
              return (
                order[key as keyof PiracyDetectionRequest] ===
                query[key as keyof PiracyDetectionRequest]
              );
            });
          })
        );
