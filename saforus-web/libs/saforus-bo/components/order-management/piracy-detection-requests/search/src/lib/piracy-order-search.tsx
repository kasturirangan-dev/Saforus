import { Box, FormControl, Typography, styled, InputBase } from '@mui/material';
import { usePiracyDetectionSearchData } from './data';
...
function PiracyDetectionSearchView() {
  const { t } = useTranslation();
  const { handleSubmit, onSubmit, control } = usePiracyDetectionSearchData();
  const { searchQuery } = useSnapshot(PiracyDetectionRequestsStore);
    ...
    <Box display={'flex'} flexDirection={'column'}>
      ...
      >
        <Typography variant="h5">
          {t('orderManagement.piracy-detection-requests.title')}
        </Typography>
        <FormControl>
          <Controller
            name={'orderNo'}
            ...
            render={({ field }) => (
              ...
              >
                ...
                <InputBase
                  ...
                  placeholder={`${t(
                    'orderManagement.piracy-detection-requests.order-no'
                  )}`}
      ...
      >
        <Typography variant="h6" color={'var(--gray-900)'}>
          {t('orderManagement.piracy-detection-requests.search-requests')}
        </Typography>
                  ...
                  render={({ field }) => (
                    <Autocomplete
                      ...
                      defaultValue={
                        serviceTypes.find(
                          (el) =>
                            `${el.value}` === `${searchQuery?.serviceType}`
                        ) || serviceTypes[0]
                      }
                      value={field.value}
                      ...
                      onChange={(event, newValue) =>
                        field.onChange(newValue?.value || serviceTypes[0])
                      }
                    />
                  )}
                />
            ...
            >
              <Controller
                ...
                render={({ field }) => (
                  // <Autocomplete
                  //   {...field}
                  //   title={`${t('orderManagement.requestor')}`}
                  //   options={requesters}
                  //   defaultValue={
                  //     requesters.find(
                  //       (el) => `${el.value}` === `${searchQuery?.userId}`
                  //     ) || requesters[0]
                  //   }
                  //   value={field.value}
                  //   isOptionEqualToValue={(option, value) =>
...
export default PiracyDetectionSearchView;
