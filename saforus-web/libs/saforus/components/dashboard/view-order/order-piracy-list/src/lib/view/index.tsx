...
function PiracyOrderListView({
  ...
  const { searchQuery } = useSnapshot(ServiceViewOrderStore);
  const [paginationModel, setPaginationModel] = useState({
    page: searchQuery.pageNoPd || 0,
  ...
  useEffect(() => {
    if (searchQuery.pageNoPd !== paginationModel.page) {
      apiRef.current?.setPage(searchQuery.pageNoPd || 0);
    }
  }, [searchQuery.pageNoPd]);
  ...
  const columns: GridColDef[] = [
    ...
    {
      field: 'details',
      headerName: `${t('piracy-order-view.table.description')}`,
      minWidth: 300,
      maxWidth: 500,
      flex: 2,
      sortable: false,
    ...
    >
      <Box sx={{ display: 'flex', gap: '10px', flexDirection: 'column' }}>
        <Typography
          ...
          color={'var(--gray-700)'}
        >
          {t('dashboard.search-orders.result-piracy-detection')}{' '}
        </Typography>
