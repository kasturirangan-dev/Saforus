...
const UsageChart: React.FC<UserOverViewProps> = ({ isLoading }) => {
  ...
  const chartData = useMemo(() => {
    ...
    return fileUsages?.usages?.map((item) => {
      ...
      const stringDate = parsedDate.toLocaleDateString(locale, {
        year: 'numeric',
        month: 'short',
        day: 'numeric',
      });
  ...
  return hasData ? (
    <StaticsBox sx={{ padding: '28px', gap: '16px', position: 'relative' }}>
      ...
      <Box>
        <Typography
          ...
          color="var(--gray-50)"
        >
          {t('apiDashboard.usage-overview.total')}
        </Typography>
        <Typography
          fontFamily="Inter"
          fontSize="36px"
    ...
    >
      ...
      <Typography color="var(--gray-700)" sx={{ whiteSpace: 'pre-line' }}>
