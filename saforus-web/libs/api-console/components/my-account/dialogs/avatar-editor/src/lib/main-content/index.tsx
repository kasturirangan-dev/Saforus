...
import { useUserAvatarData } from './data';
...
export const AvatarContent = ({
  ...
  const [previewAvatar, setPreviewAvatar] = useState(profile.avatarPreview);
  const [editAvatar, setEditAvatar] = useState(profile.avatarPreview || '');
  ...
  const handleFileDrop = (
      ...
      fileRejections.forEach((rejection) => {
        rejection.errors.forEach((error) => {
          if (error.code === 'file-invalid-type') {
            ...
            showToast.error(t('myaccount.avatar.failUploadSize'));
          }
        });
      });
      setUploadError(true);
    ...
    if (file) {
      if (!allowedFormats.includes(file.type) || file.size > maxImageSize) {
        showToast.error(t('myaccount.avatar.failUpload'));
        setUploadError(true);
        return;
      }
  ...
  const renderDefaultAvatar = useMemo(() => {
    return profile.accountName ? (
      <Typography variant="h1" fontWeight={400}>
        {getInitials(profile.accountName)}
      </Typography>
    ) : (
    ...
    >
      ...
      >
        ...
        <AvatarFooter
          ...
          isDefaultAvatar={profile.avatarPreview === ''}
